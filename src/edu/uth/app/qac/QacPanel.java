...
public class QacPanel extends JPanel implements TreeSelectionListener {
	...
	public QacPanel(QacFrame frame, String folderName) {
		this.frame = frame;
	...
	private JSplitPane createRightContents() {
		...
		JPanel textAreaScrollPane = genTextPaneScrollPane();
	...
	protected JPanel genTextPaneScrollPane() {
		final ColorTextPane textPane = frame.getColorTextPane();
		...
		textPane.setText(getUserGuide());
		//textPane.append(Color.red, getUserGuide());
	    ...
	    JButton button = new JButton(GhcImageLoader.getIcon(GhcImageLoader.DELETE_XS));
	    toolbar.add(button);
	    button.setToolTipText("clear log console");
	    ...
	    button = new JButton(GhcImageLoader.getIcon(GhcImageLoader.SAVE_XS));
	    toolbar.add(button);
	    button.setToolTipText("save log to file");
		button.addActionListener( new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				if(frame.getProject()==null) return;
				FileNameExtensionFilter [] exts = null;
				exts = new FileNameExtensionFilter [] { new FileNameExtensionFilter("TXT (*.txt)", "txt") }; 
				String fileName = frame.getProject().getFullPath()+frame.getProject().getBaseName()+"_log.txt";
				String selectedFileName = frame.saveFileUsingJFileChooser(exts, fileName);
				...
				if(selectedFileName==null) {
					...
					IOUtil.writeTextToFile(text, selectedFileName.trim());
				}			
			}
		});

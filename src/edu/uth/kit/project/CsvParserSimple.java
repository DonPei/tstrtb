...
public class CsvParserSimple {
	...
	public static void main(String[] args) throws Exception {
		...
		File file = new File("C:\\GH_DATA\\zip\\population_by_zip_2010.csv");
		CsvParserSimple obj = new CsvParserSimple();
		List<String[]> result = obj.readFile(file, 0);

		for(int i=0; i<3; i++) {
			for (String array : result.get(i)) {
...
//		}
			...
			while ((line = br.readLine()) != null) {
				...
				if (isMultiLine) {
					...
					if (pendingFieldLine != null && pendingFieldLine.length > 0) {
						...
						if(headerEnabled && k==0) {
							header = joinArrays(pendingFieldLine, csvLineInArray);
							k++;
						} else {
	...
	public String[] parseLine(String line, char separator) throws Exception {
		List<String> list = parse(line, separator, DEFAULT_QUOTE_CHAR);

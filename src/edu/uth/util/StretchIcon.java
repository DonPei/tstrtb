...
/**
 * An <CODE>Icon</CODE> that scales its image to fill the component area, excluding any
 * border or insets, optionally maintaining the image's aspect ratio by padding and
 * centering the scaled image horizontally or vertically.
 * <P>
 * The class is a drop-in replacement for <CODE>ImageIcon</CODE>, except that the
 * no-argument constructor is not supported.
 * <P>
 * As the size of the Icon is determined by the size of the component in which it is
 * displayed, <CODE>StretchIcon</CODE> must only be used in conjunction with a component
 * and layout that does not depend on the size of the component's Icon.
 * 
...
public class StretchIcon extends ImageIcon {
	...
	/**
	 * Paints the icon. The image is reduced or magnified to fit the component to which it
	 * is painted.
	 * <P>
	 * If the proportion has not been specified, or has been specified as
	 * <code>true</code>, the aspect ratio of the image will be preserved by padding and
	 * centering the image horizontally or vertically. Otherwise the image may be
	 * distorted to fill the component it is painted to.
	 * <P>
	 * If this icon has no image observer,this method uses the <code>c</code> component as
	 * the observer.
	 *
	 * @param c the component to which the Icon is painted. This is used as the observer
	...
	/**
	 * Overridden to return 0. The size of this Icon is determined by the size of the
	 * component.
	 * 
	 * @return 0
	 */
